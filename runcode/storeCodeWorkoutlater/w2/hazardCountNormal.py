    ...
    for i in range(p.breakthresh):
        ...
        if p.world[next_state[0], next_state[1]] == 0 and (
            ...
            if np.linalg.norm(next_state - target_state) <= p.thresh:
                R = p.highreward
                print(f"I reach the goal at ep {episode_no}")
            else:
                R = p.livingpenalty
...
def optpol_visualize(Qp: object) -> object:
    for i in range(p.a):
        for j in range(p.b):
            if p.world[i, j] == 0:
...
if __name__ == "__main__":
    ...
    # plt.show()
    f1.savefig('ha_count_normal_RL.png')
    ...
    # plt.show()
    f2.savefig('reward_normal_RL.png')
